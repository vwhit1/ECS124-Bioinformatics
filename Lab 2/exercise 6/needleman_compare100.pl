...
$output_file = <>;
chomp($output_file);
open (OUT, '>', $output_file);         #Open a file called 'outer1' for outputing.

print "We are taking the strings from the file now. Fancy!\n";

print "Type the input file name, please:\n";
$input_file = <>;
chomp($input_file);
# $input_file = "output100randomDNAlength10.txt";
open($INFO, '<', $input_file);


$firstline = <$INFO>;
chomp $firstline;
@string1 =  split(//, $firstline);  #split up the line into individual characters
...
# Assigning costs for LCS

print "For LCS, ";
print "Match Value is 1\n";
$Mv = 1; # not caling it V because that's what the matrix is called
print "Mismatch Cost is 0\n";
$Cm = 0;
print "Indel Cost is 0\n";
$Im = 0;
...
# Make it into a big while loop to compare the first line to all others!
